 ...
 */
dpr("function testConnectionRouting()");
 ...
 * Test that connections return appropriate connection objects.
 */
dpr("function testConnectionRoutingOverride()");
// Clone the master credentials to a slave connection.
// Note this will result in two independent connection objects that happen
// to point to the same place.
$connection_info = Database::getConnectionInfo('default');
 ...
 */
dpr("function testConnectionClosing()");
 ...
 */
dpr("function testConnectionOptions()");
$connection_info = Database::getConnectionInfo('default');
...
// options.
assertEqual($connection_info['default']['driver'], $connectionOptions['driver'], t('The default connection info driver matches the current connection options driver.'));
assertEqual($connection_info['default']['database'], $connectionOptions['database'], t('The default connection info database matches the current connection options database.'));
...
$connectionOptions = $db->getConnectionOptions();
assertIdentical($connectionOptions, $connectionOptions2, t('The default and slave connection options are identical.'));
...
// Get a fresh copy of the default connection options.
$connectionOptions = $db->getConnectionOptions();
assertNotEqual($connection_info['default']['database'], $connectionOptions['database'], t('The test connection info database does not match the current connection options database.'));
